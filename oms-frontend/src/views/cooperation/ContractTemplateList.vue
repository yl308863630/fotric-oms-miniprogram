<template>
  <div class="contract-template-list mobile-list-layout">
    ...
    <el-dialog v-model="showEditDialog" title="编辑合同模板" width="600px">
...
const editForm = ref<ContractTemplate>({
  id: 0,
  templateName: '',
  description: '',
  templateUrl: ''
...
const fetchTemplates = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/contract-templates', {
      params: {
...
const handleSaveEdit = async () => {
  ...
  try {
    await request.put(`/contract-templates/${currentEditId.value}`, editForm.value)
...
const handleDelete = (row: ContractTemplate) => {
  ElMessageBox.confirm('确认删除该模板？', '提示', {
    ...
    try {
      await request.delete(`/contract-templates/${row.id}`)
...
const handlePreview = (row: ContractTemplate) => {
  if (row.templateUrl) {
    window.open(row.templateUrl, '_blank')
  }
}

const handleDownload = (row: ContractTemplate) => {
  if (row.templateUrl) {
...
const handleFileUploadSuccess = (response: any, file: File) => {
  ...
  if (response.success || response.url) {
    ...
    console.log('addForm after:', addForm.value)
...
const handleSaveAdd = async () => {
  console.log('handleSaveAdd called')
  ...
  try {
    ...
    console.log('Saving template with relativeUrl:', relativeUrl)
    const saveData = {
    ...
    const result = await request.post('/contract-templates', saveData)
